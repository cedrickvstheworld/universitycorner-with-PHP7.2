...
if(isset($_POST['changep'])) {
    if ($_POST['currentp'] === '' ||
        ...
        $verifycurrentp = 'SELECT *FROM tblparent WHERE parentregno = :parentregno';
        $query232 = $conn->prepare($verifycurrentp);
        $query232->execute(array(
            ':parentregno' => $_POST['parentregnop'],
        ));

        if ($query232->rowCount() > 0) {
            $tblparent = $query232->fetch();
            $password = $tblparent[5];
            $pass=$_POST['currentp'];
                ...
                if (strlen($_POST['newp'])<8) {
                    ...
                    if ($_POST['newp'] !== $_POST['confirmnewp']) {
                        ...
                        if (password_verify("$passc", "$password")) {
                            ...
                            $updatepassword = 'UPDATE tblparent SET password = :newpassword WHERE parentregno = :parentregno';
                            ...
                            if ($query50) {
                                ...
                                echo "<script>document.getElementById('currentp').value=''</script>";
                                echo "<script>document.getElementById('newp').value=''</script>";
                                echo "<script>$('#changepmodalparent').modal('hide');</script>";
...
if (isset($_POST['updateinfo'])) {
    $parentregnoup = $_POST['parentregnoup'];
    $nfname = $_POST['nfname'];
    $nmname = $_POST['nmname'];
    $nlname = $_POST['nlname'];
    ...
    if (
        ...
        if ($nbirthyear < 1920 || $nbirthyear>($datestr-18) || strlen($nbirthyear) > 4) {
            ...
            //update here
            $updateinfo = 'UPDATE tblparent SET
                                 fname = :fname,
                                 mname = :mname,
                                 lname = :lname,
                                 address = :address,
